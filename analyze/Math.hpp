...
constexpr inline double fast_exp(const double x) noexcept{
  ...
  const uint64_t exp_s = Math::exp_table(*(uint64_t*)&k_R);
  return *(double*)&exp_s * ((1.0/6.0*t+1.0/2.0)*t*t+t+1);
}

constexpr inline float inv_sqrt(const float x) noexcept{
  const int i = 0x5f3759df - (*(int*)&x >> 1);
  const float y = *(float*)&i;
  return x * (1.5f - 0.5f * x * y * y);
}