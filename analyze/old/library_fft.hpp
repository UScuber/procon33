...
inline float FFT::func(const float x) const noexcept{
  //return log2(x) * 1000;
  return pow(x, 0.5) * 1000;
}

FFT fft;
constexpr int fft_duration = 256;
constexpr int max_fft_samples_num = (ans_length - sampleLength) / fft_duration;
float problem_spectrum[max_fft_samples_num][resolution];
float fft_result[resolution];
double fft_result_score[max_fft_samples_num];
int fft_samples_num;

double calc_score_fft(const Val_Type a[ans_length]) noexcept{
  rep(i, problem_length){
    const float val = (-a[i] + problem[i]) / 32768.0;
    fft.set_sample(i, val);
  }
  int cnt = 0;
  double score = 0;
  for(int pos = 0; pos+fft_duration < problem_length-sampleRange; pos += fft_duration){
    fft_result_score[cnt] = 0;
    fft.doFFT(fft_result, pos);
    rep(i, resolution){
      fft_result_score[cnt] += abs(problem_spectrum[cnt][i] - fft_result[i]);
    }
    score += fft_result_score[cnt];
    cnt++;
  }
  assert(cnt == fft_samples_num);
  return score;
}
void calc_problem_fft(){
  rep(i, problem_length){
    fft.set_sample(i, problem[i] / 32768.0);
  }
  int cnt = 0;
  for(int pos = 0; pos+fft_duration < problem_length-sampleRange; pos += fft_duration){
    fft.doFFT(problem_spectrum[cnt], pos);
    cnt++;
  }
  assert(cnt == fft_samples_num);
}
...
void read_values(){
  ...
  fft_samples_num = (problem_length - sampleRange) / fft_duration;
...
void init(){
  ...
  best_score = calc_score_fft(best_sub);
...
// DON'T MULTI THREAD
///////////////////////// FFT /////////////////////////
inline double calc_one_changed_ans_fft(const RndInfo &info) noexcept{
  ...
  for(int pos = 0; pos+fft_duration < problem_length-sampleRange; pos += fft_duration){
    // cross
    if(is_cross(info.pos, info.pos+info.len, pos, pos+sampleRange) || is_cross(pre.pos, pre.pos+pre.len, pos, pos+sampleRange)){
      for(int i = pos; i < pos + sampleRange; i++){
        const float val = (-best_sub[i] + problem[i]) / 32768.0;
        fft.set_sample(i, val);
      }
      fft_result_score[cnt] = 0;
      fft.doFFT(fft_result, pos);
