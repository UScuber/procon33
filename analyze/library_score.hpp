...
constexpr int64_t infl = (uint64_t)-1 >> 1;
...
struct Score {
  static constexpr Val_Type *problem = problem;
  constexpr Score(int64_t _a=0, int64_t _b=1) : a(_a), b(_b){}
  // -= pre, += pre + diff
  constexpr inline void add(const int i, const int pre, const int diff) noexcept{
    a += problem[i] * diff;
    b += diff * (diff + pre * 2);
  }
  constexpr inline void sub(const int i, const int pre, const int diff) noexcept{
    a -= problem[i] * diff;
    b += diff * (diff - pre * 2);
  }
  constexpr inline float val() const noexcept{
    return inv_sqrt((float)b) * a / (1 << 30);
  }
  constexpr inline double operator-(const Score &s) const noexcept{
    return (*this).val() - s.val();
  }
  constexpr inline bool operator<(const Score &s) const noexcept{
    return a < s.a * inv_sqrt((float)s.b / b);
  }
  constexpr inline bool operator>(const Score &s) const noexcept{
    return a > s.a * inv_sqrt((float)s.b / b);
  }
  friend std::ostream &operator<<(std::ostream &os, const Score &s) noexcept{
    return os << s.val();
  }
private:
  int64_t a,b;
...
constexpr Score_Type inf_score = -infl;
...
void init(){
  ...
  memset(best_sub, 0, sizeof(problem));
  // 最初は適当に値を入れておく
  rep(i, m){
    best[i].idx = i;
    best[i].pos = 0;
...
inline constexpr void calc_range_score_sub(const Val_Type a[], const Val_Type b[], const int l, const int r, Score_Type &score) noexcept{
  //rep(i, range) score += Weight(b[i] - a[i]) - Weight(b[i]);
  for(int i = l; i < r; i++) score.sub(i, b[i-l], a[i-l]);
}
inline constexpr void calc_range_score_add(const Val_Type a[], const Val_Type b[], const int l, const int r, Score_Type &score) noexcept{
  //rep(i, range) score += Weight(b[i] + a[i]) - Weight(b[i]);
  for(int i = l; i < r; i++) score.add(i, b[i-l], a[i-l]);
}
...
inline constexpr Score_Type calc_one_changed_ans(const RndInfo &info) noexcept{
  ...
  //if(max(info.pos, pre.pos) < min(info_rig, pre_rig)){
    ...
    if(info.pos <= pre.pos){
      ...
      calc_range_score_add(arr_info, best_sub+info.pos, info.pos, rightest, score);
    }
    // 左側がright
    else{
      const int rightest = min(pre_rig, info.pos);
      calc_range_score_sub(arr_pre, best_sub+pre.pos, pre.pos, rightest, score);
    }
    // 右側がright
    if(info_rig <= pre_rig){
      const int leftest = max(pre.pos, info_rig);
      const int s = max(info_rig - pre.pos, 0);
      calc_range_score_sub(arr_pre+s, best_sub+leftest, leftest, pre_rig, score);
    }
    // 右側がleft
    else{
      const int leftest = max(info.pos, pre_rig);
      const int s = max(pre_rig - info.pos, 0);
      calc_range_score_add(arr_info+s, best_sub+leftest, leftest, info_rig, score);
    }
    // middle
    ...
    rep(i, range){
      ...
      score.sub(i+leftest, best_sub[i+leftest], -arr_info[i+sl] + arr_pre[i+sr]);
    }
  }
  // not cross
  else{
    calc_range_score_add(arr_info, best_sub+info.pos, info.pos, info.pos+info.len, score);
    calc_range_score_sub(arr_pre, best_sub+pre.pos, pre.pos, pre.pos+pre.len, score);
  }
  return score;
}
